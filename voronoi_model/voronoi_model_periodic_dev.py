...
class Cell:
    ...
    def get_A(self):
        ...
        self.A = 0.5 * np.sum(
            np.absolute(self.vs[:, 0] * v_roll[:, 1] - self.vs[:, 1] * v_roll[:, 0])
        )
...
class Tissue:
    ...
    def make_init_boundary(self, L, r, noise=0.005):
        ...
        circular_mask = (self.x0[:, 0] - self.L / 2) ** 2 + (
            self.x0[:, 1] - self.L / 2
        ) ** 2 <= (r * L) ** 2
        neighs = []
    ...
    def set_interaction(
        self, W=0.16 * np.array([[2, 0.5], [0.5, 2]]), pE=0.5, c_types=None
    ):
        if c_types is None:
            nE = int(self.n_C * pE)
            N_dict = {
            ...
            for i, region in enumerate(regions):
                patches.append(
                    Polygon(
                        vertices[region],
                        True,
                        facecolor=cols[c_types_print[i]],
                        edgecolor="white",
                        alpha=0.5,
                    )
                )
    ...
    def plot_vor_boundary(self, x, ax, tri=False):
        ...
        if type(self.c_types) is list:
            ...
            if self.plot_scatter is True:
                ax.scatter(
                    x[: self.n_C, 0], x[: self.n_C, 1], color="black", zorder=1000
                )
                ax.scatter(
                    x[self.n_C :, 0], x[self.n_C :, 1], color="grey", zorder=1000
                )
            for i, region in enumerate(regions):
                patches.append(
                    Polygon(
    ...
    def generate_noise(self):
        theta_noise = np.cumsum(
            np.random.normal(0, np.sqrt(2 * self.Dr * self.dt), (self.n_t, self.n_c)),
            axis=0,
        )
    ...
    def triangulate(self, x):
        ...
        if type(self.k2s) is list:
            ...
            (self.Angles[self.v_neighbours_flat, self.k2s] + self.Angles.ravel())
            < np.pi
        )[self.b_neighbour_mask].all():
            self._triangulate(x)
        else:
    ...
    def equiangulate(self, x, mask):
        ...
        while (not mask.all()) and (k < timeout):
            ...
            self.tris[chosen_opposite_cell] = (
                opposite_cells[0],
                cells[0],
                opposite_cells[2],
            )
            self.Angles[[chosen_cell, chosen_opposite_cell]] = tri_angles_periodic(
                x, self.tris[[chosen_cell, chosen_opposite_cell]], self.L
            )
            # self.Angles = tri_angles_periodic(x,self.tris,self.L)
            self.Cents = x[self.tris]
            self.vs = self.get_vertex_periodic()
            modify_neighbours = np.concatenate(
                [
                    self.v_neighbours[chosen_cell],
                    self.v_neighbours[chosen_opposite_cell],
                ]
            )
            modify_neighbours.sort()
            self.v_neighbours[modify_neighbours] = -1
            n_neigh = get_neighbours(
                self.tris, self.v_neighbours, Range=modify_neighbours
            )
...
#                         print(self.Angles)
    ...
    def check_boundary(self, x):
        ...
        if add_extra.any():
            ...
            for k, i in enumerate(I):
                ...
                d = np.cross(xs[2] - xs[0], xs[1] - xs[0]) / np.sqrt(
                    ((xs[2] - xs[1]) ** 2).sum()
                )
                x_new = xs[0] - 2 * d * perp
                x = np.vstack((x, x_new))
        ...
        keep_mask = (
            C[self.n_C :, : self.n_C].sum(axis=1) > 0
        )  # I'm assuming this is the same thing. This removes all boundary centroids that are not connected to at least one real centroid.
        if keep_mask.any():
    ...
    def get_A_periodic(self, neighbours, vs):
        ...
        :return: self.A saves the areas of each cell
        """
        self.A = get_A_periodic(
            vs, neighbours, self.Cents, self.CV_matrix, self.L, self.n_c
        )
        return self.A
    ...
    def get_l_interface(self):
        return get_l_interface(
            self.n_v, self.n_c, self.neighbours, self.vs, self.CV_matrix, self.L
        )

    def warmup_SPV(self, print_updates=True, F_get=None):
        """
        ...
        for _ in range(self.n_warmup_steps):
            x = self.step_voronoi(x, F_get=F_get)
    ...
    def simulate(self, variable_param=False, progress_bar=False, print_updates=False):
        """
        Evolve the SPV.
        ...
        # Warm up simulation
        x = self.warmup_SPV(print_updates=print_updates, F_get=F_get)
    ...
    def step_GRN_delay(self, E, past_step, sender_idx=0):
        ...
        dE_dt = self.GRN_rhs(E, self.E_save[past_step], transition_mtx, self.args)
    ...
    def simulate_morph_delay(
        ...
        x = self.warmup_SPV(print_updates=print_updates, F_get=F_get)
        self.assign_types(
            types, type_n_c, init_vals, type_method=type_method, center=center,
        )
        E = self.init_GRN()
        ...
        for step in iterator:
            ...
            self.J = self.W[cell_i, cell_j] + self.adh_coeff * np.outer(
                self.adh_prot, self.adh_prot
            )
    ...
    def simulate_GRN(self, print_every=1000, print_updates=True):
        ...
        for i in range(n_t - 1):
            ...
            if i <= i_past:
                ...
                E = (
                    self.GRN_step(self.E_save[i - i_past], E, self.l_save[i - i_past])
                    * ~self.Sender
                    + self.Sender * self.sender_val
                )
    ...
    def GRN_step(self, A_in, A, l_matrix):
        ...
        dtA = (
            ...
            * (
                ...
                + self.alpha
                * (E_bar ** self.p)
                / (E_bar ** self.p + self.K ** self.p + (self.delT * A_in) ** self.p)
                - A
            )
        )
        A_out = A + self.dt * dtA
    ...
    def get_self_self_interface(self, nT=100):
        ...
        for j, t in enumerate(t_sel):
            ...
            h_jm1 = np.dstack(
                (
                    roll_forward(self.neighbours[:, :, 0]),
                    roll_forward(self.neighbours[:, :, 1]),
                )
            )
    ...
    def get_num_islands(self, nT=100):
        ...
        for i, t in enumerate(t_sel):
            ...
            A_islands[i], B_islands[i] = (
                connected_components(csgraph=csr_matrix(AdjA), directed=False)[0],
    ...
    def save_l_mtx(self, fname):
        ...
        """
        np.savez(
            fname, **{str(step): csr for step, csr in zip(range(self.n_t), self.l_save)}
        )
    ...
    def save_all(
        self, to_dir, prefix, index=False, df_kwargs=dict(), csv_kwargs=dict()
    ):
    ...
    for i in range(3):
        ...
        DHDR[:, i, 0, 0] = (ax * (by - cy)) / (
            ...
            * (
            ...
            2.0 * ((ay - by) * cx + ax * (by - cy) + bx * (-ay + cy)) ** 2
        )
        # dhy/drx
        DHDR[:, i, 0, 1] = (
            bx ** 2 + by ** 2 - cx ** 2 + 2 * ax * (-bx + cx) - cy ** 2
        ) / (2.0 * ((ay - by) * cx + ax * (by - cy) + bx * (-ay + cy))) - (
            (by - cy)
            * (
                (bx ** 2 + by ** 2) * (ax - cx)
                + (ax ** 2 + ay ** 2) * (-bx + cx)
        ...
        # dhx/dry
        DHDR[:, i, 1, 0] = (
            -(bx ** 2) - by ** 2 + cx ** 2 + 2 * ay * (by - cy) + cy ** 2
        ) / (2.0 * ((ay - by) * cx + ax * (by - cy) + bx * (-ay + cy))) - (
            (-bx + cx)
            * (
                (ax ** 2 + ay ** 2) * (by - cy)
...
def get_A_periodic(vs, neighbours, Cents, CV_matrix, L, n_c):
    ...
    for i in range(3):
        Neighbours = (
            np.remainder(neighbours[:, np.mod(i + 2, 3)] - Cents[:, i] + L / 2, L)
            - L / 2
        )
...
def get_F_periodic(
    ...
    for i in range(3):
        h_j[:, i] = vs
    h_jm1 = np.dstack(
        (roll_forward(neighbours[:, :, 0]), roll_forward(neighbours[:, :, 1]))
    )
    h_jp1 = np.dstack(
    ...
    l_jm1_norm, l_jp1_norm = (
        np.sqrt(l_jm1[:, :, 0] ** 2 + l_jm1[:, :, 1] ** 2),
        np.sqrt(l_jp1[:, :, 0] ** 2 + l_jp1[:, :, 1] ** 2),
    )
...
def weak_repulsion(Cents, a, k, CV_matrix, n_c, L):
    ...
    """
    CCW = np.dstack(
        (roll_reverse(Cents[:, :, 0]), roll_reverse(Cents[:, :, 1]))
    )  # np.column_stack((Cents[:,1:3],Cents[:,0].reshape(-1,1,2)))
    ...
    for i in range(3):
        F_soft += np.asfortranarray(CV_matrix[:, :, i]) @ np.asfortranarray(
            V_soft[:, i]
        )
    return F_soft
...
def get_C(n_c, CV_matrix):
    ...
    for i in range(3):
        C += np.asfortranarray(CV_matrix[:, :, i]) @ np.asfortranarray(
            CV_matrix[:, :, np.mod(i + 2, 3)].T
        )
...
def get_F_bend(n_c, CV_matrix, n_C, x, zeta):
    ...
    for i in range(3):
        C_b += np.asfortranarray(CV_matrix[n_C:, :, i]) @ np.asfortranarray(
            CV_matrix[n_C:, :, np.mod(i + 2, 3)].T
        )
    x_i = x[n_C:]
...
def dcosthetadr(ri, rj, rk):
    ...
    cos_theta = (rji[:, 0] * rki[:, 0] + rji[:, 1] * rki[:, 1]) / (
        np.sqrt(L2_ji) * np.sqrt(L2_ki)
    )
    dC_ri = (rji.T / L2_ji) * (cos_theta - np.sqrt(L2_ji / L2_ki)) + (rki.T / L2_ki) * (
        cos_theta - np.sqrt(L2_ki / L2_ji)
    )
